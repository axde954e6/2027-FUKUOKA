  ...
  return <>
    ...
    <main>
      ...
      <section className="stay-section" id="stay">
        ...
        <div className="stay-grid">{stays.map((stay) => <article className="stay-card" key={stay.name}><div className="stay-title"><div className="stay-heading"><strong>{stay.name}</strong><div className="stay-markers"><span className="badge">{stay.place}</span>{stay.payment && <span className="payment-status">{stay.payment}</span>}</div></div><div className="stay-booking-row">{stay.bookingHref ? <a className="booking-source" href={stay.bookingHref} target="_blank" rel="noreferrer">{stay.bookingLabel}</a> : <span className="booking-source">{stay.bookingLabel}</span>}</div></div><p className="stay-facts">{stay.dates}<span><strong>{stay.price}</strong>{stay.priceNote && `（${stay.priceNote}）`}</span></p><div className="links"><a className="btn" href={stay.href} target="_blank" rel="noreferrer">官方網站</a><a className="btn" href={stay.map} target="_blank" rel="noreferrer">Google Maps</a></div></article>)}</div>
      </section>
